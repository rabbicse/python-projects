...
# Get all keyframes
echo "Getting all keyframes..."
keyframes_time=$(ffprobe -hide_banner -loglevel warning \
    -select_streams v:0 -skip_frame nokey -show_frames \
    -show_entries frame=pkt_dts_time "$1" \
    | grep "pkt_dts_time=" \
    | awk -F "=" '{print $2}')
...
counter=0
echo "Extracting frames (all keyframes)..."
for i in $keyframes_time; do
    if ! [[ "$i" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        echo "Skipping invalid timestamp: $i"
        continue
    fi
